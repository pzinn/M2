...
greekLettersList = apply(greekLetters0, x -> if class x === Option then x#0 else x); -- just names
greekLetters = hashTable apply(greekLetters0,x->if class x === Option then x else x => "\\" | x)
texVariable := x -> (
    if x === "" then return "";
