...
Thing.RobustPrintMethod = mode((msg, obj) -> try toString stack { msg | ":",
    horizontalJoin("\t", silentRobustNetWithClass(60, 5, 3, obj)) } else msg)
*-
-- TODO somehow merge with my print error stuff
...
if topLevelMode =!= WebApp then (
	...
	scan(flexibleBinaryOperators, op -> (
		if not Thing#?((op,symbol =),Thing,Thing) then (
	    	    undocumented' ((op,symbol =),Thing,Thing);
	    	    installMethod((op,symbol =), Thing, Thing, (x,y,z) -> error splice (
		...
		if not Thing#?(op,Thing,Thing) then (
	    	    undocumented' (op,Thing,Thing);
	    	    installMethod(op, Thing, Thing, (x,y) -> error splice (
		    	    "no method for ",
			    if op === symbol SPACE then "adjacent objects:" else ("binary operator ",op," applied to objects:"),
			    BR{},
		    	    UL { LI ofc x, LI ofc y }
