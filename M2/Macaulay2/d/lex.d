...
recognize(file:PosFile):Word := (
     ...
     is null do (
	  getc(file);
	  empty(tokenbuf);
	  Word("invalid character",TCerror,hash_t(0),newParseinfo())
	  )
     is word:Word do ( 
...
-- with the delimiters
getstringslashes(o:PosFile):Word := (		    -- /// ... ///
     line := o.line;
     column := o.column;
     getc(o);		  -- pass '/'
     getc(o);		  -- pass '/'
...
--     pos := position(o);
     ...
     while true do (
	  ...
	  if ch == ERROR then (
	       empty(tokenbuf);
	       return Word("ERROR in string /// ... /// beginning here: " + o.file.errorMessage,TCerror,hash_t(0),newParseinfo());
	       );
	  if ch == EOF then (
	       empty(tokenbuf);
	       return Word("EOF in string /// ... /// beginning here",TCerror,hash_t(0),newParseinfo());
	       );
	  ...
	  if hexcoming > 0 then (
	       if ishexdigit(ch) then (
		    ...
		    while true do (ch2 := getc(o); if ch2 == EOF || ch2 == ERROR || ch2 == int('\n') then
		    	       return Word("expected " + tostring(hexcoming) + " more hex digits(s)",TCerror,hash_t(0),newParseinfo()););
		    )
	       )
	  else if escaped
	  then (
	       ...
	       then escaped = false
		    ...
		    empty(tokenbuf);
		    while true do (ch2 := getc(o); if ch2 == EOF || ch2 == ERROR || ch2 == int('\n') then
		    	       return Word("unknown escape sequence: \\" + char(ch),TCerror,hash_t(0),newParseinfo()););
		    );
	       )
...
errorToken(position:Position,message:string):Token := Token(Word(message,TCerror,hash_t(0),newParseinfo()),
     position.filename,
     position.line,
     position.column,
     position.line,
     ...
     dummySymbol,false);
errorToken(filename:string,line:ushort,column:ushort,loadDepth:ushort,message:string):Token := Token(Word(message,TCerror,hash_t(0),newParseinfo()),
     filename,
     line,
     column,
     line,
...
gettoken1(file:PosFile,sawNewline:bool):Token := (
     ...
     while true do (
	  rc := skipwhite(file);
	  if rc == ERROR then return errorToken(file.filename,swline,swcolumn,loadDepth,"parsing error");
	  if rc == EOF  then (
	       -- empty(tokenbuf);
	       -- while true do (ch2 := getc(file); if ch2 == EOF || ch2 == ERROR || ch2 == int('\n') then break;);
     	       return errorToken(file.filename,swline,swcolumn,loadDepth,"EOF in block comment -* ... *- beginning here");;
	       );
	  if rc == DEPRECATED then (
     	       return errorToken(file.filename,swline,swcolumn,loadDepth,"encountered disabled block comment syntax {* ... *} beginning here");
	       );
	  line := file.line;
	  column := file.column;
	  ch := peek(file);
     	  if iseof(ch) then return Token(wordEOF,file.filename, line, column, file.line, file.column, loadDepth,globalDictionary,dummySymbol,sawNewline)
	       ...
	       return Token(
		    ...
		    file.filename, line, column, file.line, file.column, loadDepth,globalDictionary,dummySymbol,sawNewline))
	       ...
	       return Token(makeUniqueWord(takestring(tokenbuf),parseWORD),file.filename, line, column, file.line, file.column, loadDepth,globalDictionary,dummySymbol,sawNewline))
	       ...
	       then (
		    ...
		    if c == int('p') then (
			 ...
			 then (
			      ...
			      empty(tokenbuf);
			      return errorToken(position(file),"precision missing in floating point constant");
			      )
			 );
	       	    c = peek(file);
		    if c == int('e') || c == int('E') then (
			 ...
			 then (
			      ...
			      empty(tokenbuf);
			      return errorToken(position(file),"exponent missing in floating point constant");
			      )
			 );
	       	    c = peek(file);
		    );
	       ...
	       s := takestring(tokenbuf);
	       return Token(Word(s,typecode,hash_t(0), parseWORD),file.filename, line, column, file.line, file.column, loadDepth,globalDictionary,dummySymbol,sawNewline))
	  else if ch == int('/') && peek(file,1) == int('/') && peek(file,2) == int('/') then (
	       return Token(getstringslashes(file),file.filename, line, column, file.line, file.column, loadDepth,globalDictionary,dummySymbol,sawNewline)
	       )
	  else if isquote(ch) then (
	       return Token(getstring(file),file.filename, line, column, file.line, file.column, loadDepth,globalDictionary,dummySymbol,sawNewline)
	       )
	       ...
	       return Token(makeUniqueWord(takestring(tokenbuf),parseWORD),file.filename, line, column, file.line, file.column, loadDepth,globalDictionary,dummySymbol,sawNewline))
	        ...
	        return Token(recognize(file),file.filename, line, column, file.line, file.column, loadDepth,globalDictionary,dummySymbol,sawNewline))
		)
	);
